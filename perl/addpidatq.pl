...
if ($titlecount[0]  == 1 ) {
	...
	if($now< $startdatetime) {
		...
		if ($startafter > 14 ) {
			...
			Schedule::At::add (TIME => "$atdateparam", COMMAND => "$toolpath/perl/recwrap.pl $recch $reclength $bitrate $tid $countno $pid $stationid $usedigital $digitalstationband $digitalch\n\n" , TAG => "$pid"."_R");
