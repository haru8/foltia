...
# 20071016 Patched by DCC-JPL Japan / foltia project / http://www.dcc-jpl.com/soft/foltia/
...
my %settings = (
  ...
  VideoHeight       => "480",
  VideoStandard     => "NTSC-M", # L135 v060406.pl MMM "NTSC",  # NTSC, PAL or SECAM
...
## start new MMM # first verify the freqency table is appropriate for the standard (NTSC, PAL, SECAM),
...
if ($settings{UsingIvtvDriver})
{
  ...
  if ($ivtvVersion > 265) # > 0.1.9
  {
    ...
    if (!$result)
    {
## L1312 macmil_co_jp MMM    die "Error calling setEndGOP($settings{CaptureLastGOP})!\n";
    }
  }
...
if ($changedSettings{codec})
{
  ...
  if (!$result)
  {
## L1334 macmil_co_jp MMM    die "Error calling setCodecInfo()!\n";
  }
}
...
if ($settings{ResetCardSettings} && !$settings{DontRecord})
{
  ...
  if ($changedSettings{codec})
  {
    ...
    if (!$result)
    {
## L1360 macmil_co_jp MMM      die "Error calling setCodecInfo()!\n";
    }
  }
...
## L1460 v060406.pl MMM   my $result = $ivtvObj->setStandard($tunerFD, $standard);
    if (!$result)
    {
## L1463 v060406.pl MMM      die "Error:  setStandard($standard) failed!\n";
    }
  }
}
