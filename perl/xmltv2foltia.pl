...
&writelog("xmltv2foltia.pl START.");
...
while(<>) {
    ...
    #登録処理はココで
    #&writelog("DEBUG call chkerase $item{'start'},$item{'channel'}");
...
sub replaceepg {
    ...
    while (@data = $sth->fetchrow_array()) {
        ...
        #&writelog("DEBUG push(\@deleteepgid,$data[0]);");
    } #end while
...
sub registdb {
    ...
    #&writelog("DEBUG $foltiastarttime:$foltiaendtime");
    ...
    #       $newepgid++;
    #   }
    #&writelog("DEBUG $currentepgid[0] /  $newepgid");
    ...
    #}else{
    #&writelog("DEBUG SKIP $foltiastarttime:$foltiaendtime");
    #}#未来じゃなければ挿入しない

} #end sub registdb

sub commitdb {
    &writelog("DEBUG commitdb() Start.");
    ...
    # 削除
    &writelog("DEBUG foltia_epg DELETE Start.");
    $sth = $dbh->prepare($stmt{'xmltv2foltia.commitdb.1'});
    foreach $delid (@deleteepgid) {
        $sth->execute( $delid ) || warn "$delid\n";
        #&writelog("DEBUG $stmt{'xmltv2foltia.commitdb.1'}/$delid");
        $i++;
    }
    &writelog("DEBUG foltia_epg DELETE End. : $i");

    # 追加
    &writelog("DEBUG foltia_epg INSERT Start.");
    ...
    &writelog("DEBUG foltia_epg INSERT End. : $i");
    $dbh->commit;
    $dbh->{AutoCommit} = 1;

    &writelog("DEBUG commitdb() End.");
} #end sub commitdb
